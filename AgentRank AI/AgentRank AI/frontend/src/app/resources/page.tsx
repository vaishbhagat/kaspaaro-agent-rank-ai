      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 px-4 md:px-0">
           {filteredArticles.map((article, i) => (
             ...
             >
               <div className={`aspect-video rounded-3xl mb-6 border border-white/10 overflow-hidden relative ${article.image}`}>
                  <div className="absolute inset-0 bg-white/5 group-hover:bg-transparent transition-colors" />
                  <div className="absolute top-4 left-4 p-2 rounded-xl glass border border-white/10 group-hover:scale-110 transition-transform">
        ...
        {/* Newsletter Section */}
        <div className="mt-32 p-1 rounded-[3rem] bg-white/5 border border-white/10 overflow-hidden relative">
           <div className="absolute -top-20 -right-20 w-64 h-64 bg-violet-600/10 blur-[100px] rounded-full" />
           <div className="p-12 md:p-20 text-center space-y-8">
              <h2 className="text-3xl font-bold text-white">Weekly AI Insights</h2>
              <p className="text-white/40 max-w-xl mx-auto">Join 5,000+ merchant leaders getting the latest AI discovery strategies in their inbox every Tuesday.</p>
              <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                 <input 
                   type="email" 
                   placeholder="Enter your email"
                   className="flex-1 px-6 py-4 rounded-2xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-violet-500/50 transition-all"
                 />
                 <button className="px-8 py-4 rounded-2xl bg-violet-600 hover:bg-violet-500 text-white font-bold transition-all shadow-lg shadow-violet-600/30">
                    Subscribe
                 </button>
              </div>
           </div>
