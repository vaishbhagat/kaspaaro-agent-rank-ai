...
import { 
  ...
  BookOpen, LogOut, ChevronLeft, ChevronRight, ChevronDown,
  Zap, Package, Search
} from 'lucide-react';
      ...
      >
          ...
          {menuItems.map((group) => (
            <div key={group.group} className="space-y-1">
              ...
              {group.items.map((item) => {
                ...
                const href = (storeParam && group.group === 'Analysis') ? `${item.href}?store=${encodeURIComponent(storeParam)}` : item.href;
                return (
                  <Link
                    key={item.name}
                    href={href}
        ...
        <div className="p-4 border-t border-white/5 space-y-4">
          ...
          <div className={`flex items-center ${collapsed ? 'justify-center' : 'gap-3'} px-2`}>
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1 flex items-center justify-between cursor-pointer hover:bg-white/5 p-1 rounded-md transition-colors">
