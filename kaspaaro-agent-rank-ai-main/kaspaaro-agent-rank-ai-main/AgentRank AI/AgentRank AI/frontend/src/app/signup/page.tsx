...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await signUp(fullName, formData.email, formData.password);
    setLoading(false);
    if (res.success) {
      router.push('/connect');
    } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-2">
                 <label className="text-[10px] font-bold text-white/50 uppercase tracking-widest px-1">First Name</label>
               ...
               <div className="space-y-2">
                 <label className="text-[10px] font-bold text-white/50 uppercase tracking-widest px-1">Last Name</label>
                 <div className="relative">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20" />
                    <input 
                      type="text" 
